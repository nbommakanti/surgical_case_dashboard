...
if uploaded_file is not None: # Only run once file is uploaded
    ...
    with all_data:
        ...
        with st.expander('', expanded=True):
            ...
            with filter_cols[0]:
                selected_first_date = st.date_input(
                    label='First date',
                    value=first_date,
            ...
            with filter_cols[1]:
                selected_last_date = st.date_input(
                    label='Last date',
                    value=last_date,
                    min_value=first_date,
                    max_value=last_date,
                )
            date_range = pd.to_datetime([selected_first_date, selected_last_date])
            with filter_cols[2]:
                filter_term = st.text_input(label='Filter')
            with filter_cols[3]:
